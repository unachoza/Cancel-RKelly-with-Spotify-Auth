...
class ProblemRK extends Component {
  ...
  render() {
    ...
    const songs = RKSongTitle.map((song, i) => {
      return (
        <div className={deleted ? 'animated zoomOut' : ''} style={{ textAlign: 'left' }} key={i}>
    ...
    return (
      <div style={{ fontSize: '20px', fontWeight: '300' }}>
        {songs}
        <button id="remove-button" onClick={(e) => this.removeSongs(playlistId, uri)}>
          Remove
        </button>
      </div>
    );
